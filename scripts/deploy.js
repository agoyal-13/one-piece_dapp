...
async function main() {
    ...
    const onePiecePersonalityDappContract = await ethers.getContractFactory("OnePieceMint");
    console.log("Deploying OnePiecePersonalityDapp...");

    // Deploy the contract with provided arguments
    const onePiecePersonalityDapp = await onePiecePersonalityDappContract.deploy(vrfCoordinatorV2Address, subId, keyHash, gasLimit);
